...
class BridgeServer(port: Int) : NanoHTTPD(port) {
    ...
    override fun serve(session: IHTTPSession): Response {
        ...
        return try {
            when {
                ...
                uri == "/chat_list" && method == Method.GET -> handleChatList()
    ...
    /**
     * GET /chat_list - 获取联系人列表（占位实现）
     */
    private fun handleChatList(): Response {
        return json(Response.Status.OK, mapOf(
            "status" to "ok",
            "message" to "Feature coming soon"
        ))
    }

    /**
     * GET /chat_history - 获取会话历史（占位实现）
     */
